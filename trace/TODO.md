...
* [x] For each kernel, list p50, max, and total time separately, instead of summing them up. On the left side, show `[num_instances] kernel_id` (e.g. `[25] flash_attn`)
* [x] Sort the rows by max execution time descending.